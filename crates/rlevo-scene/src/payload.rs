...
pub struct Point2 {
    /// Horizontal coordinate, in the family's own frame.
    pub x: f32,
    /// Vertical coordinate, in the family's own frame.
    pub y: f32,
}
...
#[non_exhaustive]
pub enum BodyKind {
    /// The main chassis or torso — the body a viewer reads as "the agent".
    Hull,
    /// A rolling contact body, drawn as a circle regardless of its polygon.
    Wheel,
    /// An articulated limb segment.
    Leg,
    /// A lifting or control surface.
    Wing,
    /// Static terrain the agent moves over. Never drawn as part of the agent.
    Ground,
    /// A target region the episode is scored against.
    Goal,
    /// Anything the six named classes do not describe. Drawn with the neutral
...
pub enum GridDir {
    /// Facing `+x`.
    East,
    /// Facing `+y`. Grid rows increase downward, so this is visually down.
    South,
    /// Facing `-x`.
    West,
    /// Facing `-y`. Visually up.
    North,
}
...
pub enum GridDoorState {
    /// Passable, and drawn as an opening.
    Open,
    /// Shut but not locked — passable by a `Toggle` action with no key.
    Closed,
    /// Shut and requiring a key of the door's own colour.
    Locked,
}
...
pub struct CardTable {
    /// The player's hand, in blackjack face values.
    pub player_cards: Vec<u8>,
    /// The player's hand total, with any ace already counted the way
    /// `usable_ace` reports.
    pub player_total: u8,
    /// Whether an ace is currently being counted as 11 rather than 1.
    pub usable_ace: bool,
    /// The dealer's full hand. Populated for post-episode review; during play
    /// the hole card is not shown to the agent.
    pub dealer_cards: Vec<u8>,
    /// The dealer's single face-up card — what the agent actually observes.
...
pub enum TabularLayout {
    /// A rectangular cell grid — `FrozenLake`, `CliffWalking`, `Taxi`.
    Grid(TabularGrid),
    /// A card table — Blackjack.
    Cards(CardTable),
}
