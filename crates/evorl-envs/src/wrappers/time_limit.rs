...
impl<E> TimeLimit<E> {
    ...
    pub fn new(env: E, max_steps: usize) -> Self {
        Self { inner: env, max_steps, steps: 0 }
    }
...
impl<const D: usize, const SD: usize, const AD: usize, E, Obs, Rew>
    Environment<D, SD, AD> for TimeLimit<E>
where
    E: Environment<D, SD, AD, ObservationType = Obs, RewardType = Rew,
    ...
    impl Environment<1, 1, 1> for StubEnv {
        ...
        fn step(&mut self, _action: StubAction) -> Result<Self::SnapshotType, EnvironmentError> {
            ...
            if self.pos >= self.goal {
                Ok(SnapshotBase::terminated(StubObs { pos: self.pos }, ScalarReward(1.0)))
            } else {
                Ok(SnapshotBase::running(StubObs { pos: self.pos }, ScalarReward(0.0)))
            }
        }
    }
