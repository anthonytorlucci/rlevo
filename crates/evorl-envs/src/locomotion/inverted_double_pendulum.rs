...
impl Environment<1, 1, 1> for InvertedDoublePendulum<Rapier3DBackend> {
    ...
    fn step(
        ...
        let healthy = self
            .config
            .healthy
            .is_healthy(y_tip, 0.0, &obs.0);

        let alive = if healthy { self.config.alive_reward } else { 0.0 };
        let distance = -self.config.x_tip_weight * x_tip * x_tip
            - (y_tip - self.config.y_tip_target).powi(2);
...
mod tests {
    ...
    fn alive_bonus_paid_only_while_healthy() {
        ...
        for _ in 0..2000 {
            ...
            if snap.is_terminated() {
                terminated_alive =
                    Some(snap.metadata().unwrap().components[METADATA_KEY_ALIVE]);
                break;
            }
        }
        assert_eq!(terminated_alive, Some(0.0), "alive must be 0 once unhealthy");
    }
    ...
    fn constraint_force_is_finite() {
        ...
        for _ in 0..5 {
            let snap = env
                .step(InvertedDoublePendulumAction::new(0.5))
                .unwrap();
            assert!(
    ...
    fn determinism_across_reset() {
        let cfg = InvertedDoublePendulumConfig { seed: 123, ..Default::default() };
    ...
    fn invalid_action_is_error() {
        let mut env = InvertedDoublePendulumRapier::with_config(deterministic_cfg());
        env.reset().unwrap();
        assert!(env.step(InvertedDoublePendulumAction::new(f32::NAN)).is_err());
    }
    ...
    fn obs_is_finite_after_rollout() {
        ...
        for _ in 0..50 {
            let snap = env
                .step(InvertedDoublePendulumAction::new(0.1))
                .unwrap();
