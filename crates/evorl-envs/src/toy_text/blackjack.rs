...
impl Default for BlackjackVariant {
    fn default() -> Self {
        BlackjackVariant::Standard { natural_pays_bonus: false }
    }
}
...
impl Default for BlackjackConfig {
    fn default() -> Self {
        Self { variant: BlackjackVariant::Standard { natural_pays_bonus: false }, seed: 0 }
    }
}
    ...
    fn step(&mut self, action: BlackjackAction) -> Result<Self::SnapshotType, EnvironmentError> {
        ...
        match action {
            ...
            BlackjackAction::Stick => {
                ...
                // Natural 3:2 bonus.
                let pays_natural =
                    matches!(self.config.variant, BlackjackVariant::Standard { natural_pays_bonus: true });
                if pays_natural
                    && (reward - 1.0).abs() < 1e-6
...
mod tests {
    ...
    #[test]
    fn obs_encoding() {
        let obs = BlackjackObservation { player_sum: 18, dealer_showing: 10, usable_ace: 0 };
    ...
    fn sab_player_natural_wins_one() {
        let cfg =
            BlackjackConfig::builder().variant(BlackjackVariant::SuttonBarto).seed(0).build();
        let mut env = Blackjack::with_config(cfg);
        env.reset().unwrap();
    ...
    #[test]
    fn determinism() {
        let cfg = BlackjackConfig { variant: BlackjackVariant::default(), seed: 99 };
        let run = || {
        ...
        let a = run();
        let b = run();
        assert!((a - b).abs() < 1e-5, "same seed must give same rewards; got {a} vs {b}");
    }
}
