...
pub enum FrozenMapSpec {
    Preset(FrozenPreset),
    Custom(Vec<String>),
    Random { nrow: usize, ncol: usize, frozen_prob: f32 },
}

impl Default for FrozenMapSpec {
    fn default() -> Self {
        FrozenMapSpec::Random { nrow: 8, ncol: 8, frozen_prob: 0.8 }
    }
}
...
impl Default for RewardSchedule {
    fn default() -> Self {
        Self { goal: 1.0, hole: 0.0, frozen: 0.0 }
    }
}
...
fn parse_map(rows: &[&str]) -> Result<ResolvedMap, MapError> {
    ...
    for (ri, row) in rows.iter().enumerate() {
        if row.len() != ncol {
            return Err(MapError::RowLengthMismatch { row: ri, got: row.len(), expected: ncol });
        }
        for (ci, ch) in row.chars().enumerate() {
            match Tile::try_from(ch) {
                ...
                Err(()) => return Err(MapError::InvalidTile { row: ri, col: ci, ch }),
            }
        }
    }
    ...
    Ok(ResolvedMap { tiles, nrow, ncol, start_pos })
}
...
fn bfs_reachable(tiles: &[Tile], nrow: usize, ncol: usize, start: usize) -> Result<(), MapError> {
    ...
    if goal_found { Ok(()) } else { Err(MapError::GoalUnreachable) }
}
...
fn generate_random_map(
    ...
    for _ in 0..MAX_RETRIES {
        ...
        if bfs_reachable(&tiles, nrow, ncol, start).is_ok() {
            return Ok(ResolvedMap { tiles, nrow, ncol, start_pos: start });
        }
    }
...
impl TryFrom<(u16, u8, u8)> for FrozenLakeState {
    ...
    fn try_from((id, nrow, ncol): (u16, u8, u8)) -> Result<Self, Self::Error> {
        ...
        if id >= n {
            return Err(StateError::InvalidData(format!("FrozenLakeState id {id} out of [0,{n})")));
        }
        Ok(FrozenLakeState { row: (id / ncol as u16) as u8, col: (id % ncol as u16) as u8, nrow, ncol })
    }
}
...
impl State<1> for FrozenLakeState {
    ...
    fn observe(&self) -> FrozenLakeObservation {
        FrozenLakeObservation { state_id: self.state_id() }
    }
...
impl FrozenLake {
    ...
    fn resolve_map(spec: &FrozenMapSpec, rng: &mut StdRng) -> Result<ResolvedMap, MapError> {
        match spec {
            ...
            FrozenMapSpec::Random { nrow, ncol, frozen_prob } => {
...
impl Environment<1, 1, 1> for FrozenLake {
    ...
    fn reset(&mut self) -> Result<Self::SnapshotType, EnvironmentError> {
        ...
        if let FrozenMapSpec::Random { nrow, ncol, frozen_prob } = &self.config.map.clone() {
    ...
    fn step(&mut self, action: FrozenLakeAction) -> Result<Self::SnapshotType, EnvironmentError> {
        ...
        let (nr, nc) =
            apply_action(self.state.row, self.state.col, effective, self.state.nrow, self.state.ncol);
        self.state.row = nr;
        self.state.col = nc;
        ...
        match tile {
            ...
            _ => Ok(SnapshotBase::running(obs, ScalarReward(self.config.reward_schedule.frozen))),
        }
    }
}
...
mod tests {
    ...
    fn reward_schedule_customisable() {
        let cfg = FrozenLakeConfig::builder()
            .map(FrozenMapSpec::Preset(FrozenPreset::Four4x4))
            .is_slippery(false)
            .reward_schedule(RewardSchedule { goal: 100.0, hole: -10.0, frozen: 0.0 })
            .seed(0)
            .build();
    ...
    fn slippery_mean_direction_differs_from_action() {
        ...
        for _ in 0..n {
            env.state = FrozenLakeState { row: 2, col: 2, nrow: 8, ncol: 8 };
    ...
    fn success_rate_distribution_at_0_75() {
        ...
        for _ in 0..n {
            env.state = FrozenLakeState { row: 4, col: 4, nrow: 8, ncol: 8 };
            env.step(FrozenLakeAction::Right).unwrap();
