...
impl TryFrom<u16> for CliffWalkingState {
    ...
    fn try_from(id: u16) -> Result<Self, Self::Error> {
        ...
        Ok(CliffWalkingState { row: (id / NCOL as u16) as u8, col: (id % NCOL as u16) as u8 })
    }
}
...
impl State<1> for CliffWalkingState {
    ...
    fn observe(&self) -> CliffWalkingObservation {
        CliffWalkingObservation { state_id: self.state_id() }
    }
...
impl CliffWalking {
    pub fn with_config(config: CliffWalkingConfig) -> Self {
        ...
        Self { state: CliffWalkingState { row: START.0, col: START.1 }, config, rng }
    }
...
impl Environment<1, 1, 1> for CliffWalking {
    ...
    fn reset(&mut self) -> Result<Self::SnapshotType, EnvironmentError> {
        self.state = CliffWalkingState { row: START.0, col: START.1 };
    ...
    fn step(&mut self, action: CliffWalkingAction) -> Result<Self::SnapshotType, EnvironmentError> {
        ...
        if is_cliff(nr, nc) {
            ...
            return Ok(SnapshotBase::running(self.state.observe(), ScalarReward(-100.0)));
        }
        ...
        if (nr, nc) == GOAL {
            Ok(SnapshotBase::terminated(self.state.observe(), ScalarReward(-1.0)))
        } else {
            Ok(SnapshotBase::running(self.state.observe(), ScalarReward(-1.0)))
        }
    }
}
...
mod tests {
    ...
    fn cliff_step_teleports_and_costs_100() {
        ...
        assert_eq!(env.state.state_id(), CliffWalkingState::from((3u8, 0u8)).state_id());
    }
    ...
    fn off_grid_stays_in_place() {
        ...
        assert_eq!(env.state.state_id(), snap_before, "off-grid move must be no-op");
    }
    ...
    fn slippery_distribution_matches_1_3() {
        let cfg = CliffWalkingConfig::builder().is_slippery(true).seed(7).build();
        ...
        assert!((p_right - 1.0 / 3.0).abs() < tol, "intended slip p={p_right}");
    ...
    #[test]
    fn determinism() {
        let cfg = CliffWalkingConfig::builder().is_slippery(true).seed(3).build();
        let run = || {
