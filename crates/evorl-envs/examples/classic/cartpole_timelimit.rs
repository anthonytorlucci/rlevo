...
use evorl_envs::classic::{CartPole, CartPoleAction, CartPoleConfig};
...
fn main() {
    println!(
        "CartPole + TimeLimit({TIME_LIMIT}) | episodes={NUM_EPISODES}"
    );
    ...
    for _ in 0..NUM_EPISODES {
        ...
        loop {
            ...
            steps += 1;
            if snap.is_done() {
                if snap.is_terminated() { terminated += 1; }
                if snap.is_truncated()  { truncated  += 1; }
                break;
            }
        }
    ...
    assert_eq!(terminated + truncated, NUM_EPISODES, "every episode must end");
}
