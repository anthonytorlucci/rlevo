...
use evorl_envs::classic::{Pendulum, PendulumAction, PendulumConfig};
...
fn main() {
    println!(
        "Pendulum random-policy rollout | time_limit={TIME_LIMIT} episodes={NUM_EPISODES}"
    );
    ...
    let var = total_rewards.iter().map(|r| (r - mean).powi(2)).sum::<f32>() / n;
    let min = total_rewards.iter().copied().fold(f32::INFINITY, f32::min);
    let max = total_rewards.iter().copied().fold(f32::NEG_INFINITY, f32::max);
    println!();
