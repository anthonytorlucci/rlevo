...
fn main() {
    ...
    println!(
        "  observation   = [cart_pos, cart_vel, pole_angle, pole_ang_vel]  shape={:?}",
        <CartPoleObservation as Observation<1>>::shape()
    );
    println!(
        "  actions       = Left | Right  count={}",
        CartPoleAction::ACTION_COUNT
    );
    println!("  reward        = +1 per step until failure");
    println!(
        "  termination   = |cart_pos| > {} m  or  |pole_angle| > {:.4} rad ({:.1}°)",
        cfg.x_threshold,
