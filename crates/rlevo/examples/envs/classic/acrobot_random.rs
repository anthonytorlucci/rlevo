...
use rlevo_environments::classic::{Acrobot, AcrobotAction, AcrobotConfig, BookDynamics};
...
fn main() {
    ...
    println!(
        "  observation   = [cos θ1, sin θ1, cos θ2, sin θ2, θ̇1, θ̇2]  shape={:?}",
        <rlevo_environments::classic::AcrobotObservation as Observation<1>>::shape()
    );
    println!(
        "  actions       = TorqueNeg(-1) | TorqueZero(0) | TorquePos(+1)  count={}",
