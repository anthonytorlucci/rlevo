...
//! (`best_fitness`, `mean_fitness`, …) which feeds the M4 metric
...
//! [`PopulationObserver`] is the structured callback the M8.1 report
//! producer (`rlevo_benchmarks::record::PopulationReporter`) attaches
...
//! field, which is the hybrid driver's responsibility (M8.2).
//!
...
///   `Strategy::diversity` extension fills it in (M8.2).
/// - `best_genome_digest` and `parents_of_best` are emitted empty /
///   `None` until per-strategy digest + parent-tracking lands (M8.2,
///   feeds the lineage DAG panel).
...
/// be shared across rayon worker threads — same shape as the M4
