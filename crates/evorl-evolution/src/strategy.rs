    ...
    F: BatchFitnessFn<B, S::Genome>,
{
    ...
    fn reset(&mut self) -> Self::Observation {
        ...
        self.state = Some(self.strategy.init(&self.params, &mut self.rng, &self.device));
    }
    ...
    fn step(&mut self, _: Self::Action) -> BenchStep<Self::Observation> {
        ...
        let (new_state, metrics) = self.strategy.tell(
            &self.params,
            population,
            fitness,
            state,
...
mod tests {
    ...
    fn harness_runs_one_generation() {
        ...
        let mut harness = EvolutionaryHarness::<TestBackend, _, _>::new(
            strategy, params, FortyTwo, 1, device, 5,
        );
        harness.reset();
