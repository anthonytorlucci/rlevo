...
impl Environment<1, 1, 1> for RandomWalkEnv {
    ...
    fn new(_render: bool) -> Self {
        Self {
            state: WalkState { position: Self::START },
            steps: 0,
        }
    }

    fn reset(&mut self) -> Result<Self::SnapshotType, EnvironmentError> {
        self.state = WalkState { position: Self::START };
        self.steps = 0;
        Ok(SnapshotBase::running(self.state.observe(), WalkReward::zero()))
    }
    ...
    fn step(&mut self, action: WalkAction) -> Result<Self::SnapshotType, EnvironmentError> {
        ...
        if next >= Self::GOAL {
            self.state = WalkState { position: Self::GOAL };
            return Ok(SnapshotBase::terminated(
                self.state.observe(),
                WalkReward(1.0),
            ));
...
fn replay_buffer_sample_batch_tensor_shapes() {
    ...
    let mut buffer = PrioritizedExperienceReplayBuilder::<
        1,
        1,
        WalkObservation,
        WalkAction,
    ...
    for i in 0..20 {
        let action = if i % 2 == 0 { WalkAction::Right } else { WalkAction::Left };
...
fn agent_stats_tracks_episodes_and_sliding_window() {
    ...
    stats.record(EpisodeResult { score: 5.0, duration: 10 });
    stats.record(EpisodeResult { score: 8.0, duration: 15 });
    stats.record(EpisodeResult { score: 3.0, duration: 7 });
