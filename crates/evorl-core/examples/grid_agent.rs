...
use evorl_core::action::{DiscreteAction, MultiDiscreteAction};
...
impl State<1> for AgentState {
    ...
    fn observe(&self) -> AgentObservation {
        AgentObservation { x: self.x, y: self.y, facing: self.facing }
    }
...
impl Interact {
    ...
    fn name(self) -> &'static str {
        match self { Self::Skip => "skip", Self::Toggle => "toggle" }
    }
}
...
fn step(state: &AgentState, action: MoveAction) -> AgentState {
    match action {
        MoveAction::TurnLeft  => AgentState { facing: state.facing.turn_left(),  ..state.clone() },
        MoveAction::TurnRight => AgentState { facing: state.facing.turn_right(), ..state.clone() },
        MoveAction::Forward   => {
            let (dx, dy) = state.facing.delta();
            AgentState { x: state.x + dx, y: state.y + dy, ..state.clone() }
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // ── 1. State ────────────────────────────────────────────────────────────
    println!("=== State ===");
    let state = AgentState { x: 2, y: 3, facing: Facing::East, width: 5, height: 5 };
    let oob   = AgentState { x: 5, y: 0, facing: Facing::North, width: 5, height: 5 };
    ...
    println!("AgentObservation::shape()    = {:?}", AgentObservation::shape());
    ...
    println!("tensor                       = {:?}", tensor.clone().into_data());
    let recovered =
        <AgentObservation as TensorConvertible<1, DemoBackend>>::from_tensor(tensor)?;
    ...
    let mut s = AgentState { x: 1, y: 3, facing: Facing::North, width: 5, height: 5 };
    let sequence = [
    ...
    for action in sequence {
        ...
        println!(
            "  ({},{}) {:?}  +  {:?}  →  ({},{}) valid={}",
            obs_before.x, obs_before.y, obs_before.facing,
            action, next.x, next.y, valid,
        );
        if valid {
            s = next;
        } else {
