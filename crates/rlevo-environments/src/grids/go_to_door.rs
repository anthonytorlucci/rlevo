...
impl HostRow<3> for GoToDoorObservation {
    ...
    /// `VIEW_SIZE * VIEW_SIZE * GO_TO_DOOR_OBS_CHANNELS` values as `f32` on
    /// every call to prove something the payload type already guarantees.
...
impl rlevo_scene::payload::GridPayloadSource for GoToDoorEnv {
