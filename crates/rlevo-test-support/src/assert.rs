...
///
/// The `>=` counterpart to [`assert_improves_over_random`], used by the discrete "reaches N"
/// convergence checks (e.g. `CartPole` avg `$\geq 100$`) where the target is an absolute floor
/// rather than a margin over a random baseline.
///
/// # Panics
///
