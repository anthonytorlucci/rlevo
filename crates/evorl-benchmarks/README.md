...
Reproducible benchmarking harness for the `burn-evorl` workspace.
...
`BenchEnv` is a narrow, object-safe environment trait intentionally lighter than `evorl_core::Environment`. It avoids const-generic threading so that heterogeneous environments can be boxed and dispatched at runtime.
...
```toml
[dependencies]
evorl-benchmarks = { path = "../evorl-benchmarks" }
```

### Minimal example — ε-greedy bandit

```rust
use evorl_benchmarks::{
...
cargo run -p evorl-benchmarks --example tabular_bandit

# Hand-rolled genetic algorithm minimising the 10-D Rastrigin function
cargo run -p evorl-benchmarks --example ga_rastrigin
```
...
- Rastrigin, L. A. (1974). *Systems of extreme control*. Nauka, Moscow. — original formulation of the Rastrigin function `f(x) = A·n + Σ(xᵢ² − A·cos(2πxᵢ))` used in the `ga_rastrigin` example and `evorl-envs::benchmarks::rastrigin`.

- Sutton, R. S., & Barto, A. G. (2018). **Reinforcement Learning: An Introduction** (2nd ed.). MIT Press. — sample-average and ε-greedy methods demonstrated in `tabular_bandit` follow Chapter 2 of this text. [Online edition](http://incompleteideas.net/book/the-book-2nd.html)
