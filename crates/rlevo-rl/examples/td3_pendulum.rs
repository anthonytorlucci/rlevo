...
fn main() {
    ...
    // Two critics deliberately share construction but are initialised from
    // independent RNG streams inside Burn, giving TD3 the diverse pair that
    // makes `min(q1, q2)` meaningful.
    ...
    println!(
        "td3_pendulum: final avg reward over last {} episodes: {avg:.2}",
