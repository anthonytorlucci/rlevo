...
use rlevo_envs::classic::{Pendulum, PendulumAction, PendulumConfig, PendulumObservation};
...
fn main() {
    ...
    println!("  observation   = [cos θ, sin θ, θ̇]  shape={:?}",
        <PendulumObservation as Observation<1>>::shape());
    println!("  action        = continuous torque in [-{0}, {0}]  shape=[1]", cfg.max_torque);
    ...
    println!();
    println!("Terminations: {terminations}/{NUM_EPISODES} (Pendulum never terminates intrinsically)");
