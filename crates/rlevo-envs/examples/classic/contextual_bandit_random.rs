...
//!
//! ```bash
//! cargo run -p rlevo-envs --example contextual_bandit_random
//! ```
...
fn main() {
    ...
    println!(
        "ContextualBandit<{C},{K}> random-policy rollout (seed={SEED}, steps={NUM_STEPS})"
    );
    println!();
    println!("True per-context arm means q*(c, a):");
    for (c, row) in env.arm_means().iter().enumerate() {
        let formatted: Vec<String> =
            row.iter().map(|m| format!("{m:+.2}")).collect();
