...
- The workspace contains exactly these crates, each with a single, bounded responsibility:
  ...
  - `rlevo-evolution` — evolutionary strategy algorithms. Depends on `rlevo-core` (for `BenchEnv`, `FitnessEvaluable`, `Landscape`, `SeedStream`). Depends on `parking_lot` for `SharedPopulationObserver`.
