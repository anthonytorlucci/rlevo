...
- **No production crate depends on viz deps.** `rlevo-core`, `rlevo-environments`, `rlevo-reinforcement-learning`, `rlevo-evolution`, and `rlevo-hybrid` must not depend (production *or* dev) on `ratatui`, `crossterm`, `leptos`, `axum`, `wasm-bindgen`, or any chart crate. `rlevo-benchmarks` is the sole exception, and only behind its `tui` / `report` / `record` feature gates.
  ...
  - **Visualisation is opt-in, and the opt-in must actually work.** A researcher gets `ratatui` when they ask for it — by enabling `rlevo-benchmarks/tui` — and never otherwise. Note that the umbrella's `viz-tui` / `viz-report` features do *not* currently deliver this to an external consumer: they forward to `rlevo-benchmarks`, which `rlevo` lists as a **dev**-dependency, so the forwarding is inert outside the workspace. Advertising a feature that does nothing is worse than not offering one.
