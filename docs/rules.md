...
- The workspace contains exactly these crates, each with a single, bounded responsibility:
  ...
  - `rlevo-benchmarks` — paradigm-neutral evaluation harness (light-touch dep on `rlevo-core`). Has optional `tui` (ratatui live dashboard), `report` (static-HTML Leptos viewer), and `record` (EpisodeRecord file writer) features; this is the **only** production crate permitted to carry viz deps, and only behind those feature gates.
  ...
  - `rlevo` — public API re-export aggregator; includes `rlevo-core`, `rlevo-reinforcement-learning`, `rlevo-evolution`, and `rlevo-hybrid`.
...
- `snake_case`, **$\leq 24$ characters**, no abbreviations that drop letters (`constraints`, never `contraints`).
...
- Every `Strategy`, operator, shaping rule, local searcher, and metric  aggregation in `rlevo-evolution` works purely in **canonical (maximise)**  space and is **sense-unaware** — it never sees an `ObjectiveSense`. Best is the  largest value; the worst-value sentinel is `f32::NEG_INFINITY`. Fitness hygiene  is one rule: $\text{NaN} \to -\infty$ (worst), $+\infty \to \text{f32::MAX}$ (still ranks  top, and finite so it can be compared, stored, and summed *at all* — it bounds  the **value**, not a reduction over values), $-\infty$ passes through.
...
- **Sanitise fitness before comparing it.** `total_cmp` alone is *not* enough for  fitness ordering: Rust's `f32::NAN` is a *positive* NaN, so `total_cmp` ranks it  as the **maximum** — a raw `fit[b].total_cmp(&fit[a])` descending sort would put  a `NaN`-fitness member *first* (as the best). Under maximise-native `NaN` is the  **worst**, so map it to $-\infty$ first with `sanitize_fitness` (the single rule of  §Optimisation direction) and then `total_cmp`:
  ```rust
...
- **Reduce sanitized fitness in `f64`.** Sanitizing bounds a value; only the  accumulator width bounds a *reduction*. `f32::MAX` is finite, so it joins a  sum, and `f32::MAX + f32::MAX == f32::INFINITY` — two saturated members blow an  `f32` mean to $+\infty$, and one blows an `f32` variance. So: **a  reduction over sanitized fitness accumulates in `f64` and narrows to `f32` at  most once, after the reduction**. "Reduction" is any fold over  fitness magnitudes — sum, mean, variance, weighted sum — and it binds  **transitively**: a stored `f32` field holding a sum or mean of fitness is  itself a fitness magnitude, so reducing over *it* is covered (this is exactly  `allocate_offspring`). Use `fitness::sanitized_mean` / `fitness::sanitized_sum`  rather than hand-rolling the accumulator. Ordering, single-value comparison,  and argmax are **not** covered and stay `f32` — saturation is order-preserving,  so the sanitize-then-`total_cmp` bullet above is complete for them. A reduction  and the terms compared against it must be sanitized by the **same expression**: a sanitized total with an unsanitized numerator yields an infinite share, and  $\infty\text{.floor() as usize}$ saturates to `usize::MAX`.
...
| `ImportanceExponent::new(b)` | `b` not finite or $\notin [0, 1]$ — use `try_new` for runtime-derived values |
...
| `Slot::get()` / `Slot::step_with(opt, lr, grads)` | the slot is poisoned: an earlier `step_with` unwound inside `Optimizer::step` and lost the module. Every agent method that reads a trainable network inherits this through `get` |
...
| `SimulatedAnnealingParams::with_cooling` | `Geometric` $\text{factor} \notin (0, 1)$; `Linear` `delta` not finite or not `> 0` |
...
| `ops::replacement::elitist` | `k > pop_size` or $\text{pop\_size} - k > \text{offspring count}$ |
...
`Slot::get` / `Slot::step_with` is the one row whose condition is a state rather than an argument. The state is only reachable through an earlier programming error: a panic inside `Optimizer::step` (a shape mismatch, a device OOM) that has already unwound once. The module was moved into `step` and dropped during that unwind, so there is nothing to return an `Err` about. The agent has no network left, and the remedy is to rebuild it. `Slot::is_poisoned` is the non-unwinding check. `Slot`'s own docs say why a drop guard, `catch_unwind` or a per-step clone cannot close the window.
...
- A config **value** must be finite — `NaN` and $\pm\infty$ are rejected as  `ConstraintKind::NotFinite`, checked before every other float constraint.  A config **bound** may be $\pm\infty$:  `in_range(C, f, 0.0, f64::INFINITY, x)` is the correct spelling of  "non-negative, unbounded above", and is deliberately the *only* spelling —  there is no `finite_*` helper beside it. A half-open *range* belongs in `Bounds`, not in `ordered`.
...
| Integration test | **$\geq 2$ crates** together                                      | `crates/rlevo/tests/` (flat)                   | `cargo test -p rlevo`                          |
...
- **The rustdoc lints `broken_intra_doc_links`, `private_intra_doc_links` and `redundant_explicit_links` are `deny`, not `warn`** — the only lints in the tables above the `warn` baseline. A dangling doc link is a factual error about the API (`replay/config.rs` cited a `ReplayStrategy` trait method as an inherent one), and it is silent at every other level. A link to a private item is the same failure seen from the reader's side: the page it points at never renders, so public docs that defer to it ("see `Slot` for why") explain nothing. The workspace's doc-warning pile has been cleared to zero, and at `warn` a new warning would be one line in a clean build that nothing fails on. `deny` keeps it at zero. A private link is fixed by publishing the target, by moving the explanation somewhere public, or by dropping the link and keeping the backticks. Never fix it with an `#[allow]`.
