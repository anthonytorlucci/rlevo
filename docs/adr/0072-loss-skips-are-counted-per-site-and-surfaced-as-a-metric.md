...
[0056](0056-non-finite-loss-skip-and-warn-guard.md) §Decision 1 (the guard's
type shape) and §Decision 3 (the surfacing clause and the one-shot `warn!`
latch).** Following the partial-supersession precedent of ADR
[0033](0033-share-splitmix64-mixer-across-core-and-evolution.md) (which
partially supersedes ADR 0004 §6) and ADR
[0062](0062-grid-layout-fidelity-and-no-dead-rng.md) (ADR 0029): the superseded
...
- **0056 §Decision 2** — the guard sits before `backward()`, never inside
  `Slot::step_with`.
- **0056 §Decision 4** — it runs unconditionally in release; no
  `debug_assert`/config gate. Rejected by name for the fifth time (§Rejected
  alternatives).
- **0056 §Decision 5** — 8 agents, one guard per loss site, **17 sites**.
  Re-verified against the tree, site by site, in §Context 3.
- **0056 §Decision 3's *skip* semantics** — the skip fires on **every**
  non-finite occurrence, and the return value is never gated on the warning
  schedule. Reaffirmed **verbatim** in §Decision 1, because that is exactly the
  clause a reader skimming this ADR's headline ("the latch is gone") is most
  likely to break.
- **The whole of 0056 §Consequences & honest limits**, including the
  loss-level-proxy limit and the deferral of finite-loss→NaN-gradient to #328.
  Carried forward and restated in §Consequences.
...
[0059](0059-target-update-cadence-counts-gradient-updates.md) §Decision 4** —
that clause makes the update counters advance *unconditionally, including on a
skip*; this ADR ships the counter that makes the difference between attempts and
applications recoverable, and changes nothing about 0059.
...
**not** test-gated; 0056 §1's `#[cfg(test)] warning_fired()` is deleted, not
reimplemented. Each of the 8 agents exposes one `#[must_use]` public accessor per
...
ADR 0056 §Decision 3 closed with:
...
about the *weights*, and it is true — 0056 §2 guarantees the parameters are never
touched, so the next minibatch genuinely can recover. But the quantity #346 cares
about is not weight health, it is **throughput**. A skipped update is a step of
the run's budget that bought nothing, and skipped throughput does not heal; it
accumulates. 0065's own §Decision 3 reached the right general rule one paragraph
later — *magnitude, not occurrence, is the operational fact* — and then failed to
apply it to the guard sitting directly above it in the same file.
...
0056 §Decision 5's enumeration was re-derived from the tree
...
17 total, matching 0056 §5 exactly. **Five agents are multi-site, three are
single-site**, and that split — not the count — is what §Decision 3 turns on.
SAC's α guard (#184, `sac_alpha.rs`) is a different type in a different module
and is untouched here, exactly as 0056 §Consequences ruled.
...
**And separately**, ADR 0071 §Reopen trigger 1 fences the destination:

> **A twelfth accessor of any kind on `AgentStats`** — deliberately tighter than
> 0070 §Reopen trigger 1's "a third windowed mean". At eleven … the flat surface
> is at the limit of what naming can carry.

Both facts point the same way, and the order matters: the scope mismatch is the
*reason*, the trigger is the *fence*. §Decision 3 records the outcome explicitly
so that ADR 0071's trigger is neither fired nor weakened by this change.

### 5. What ADR 0059 left legible-in-principle and illegible-in-practice

0059 §Decision 4 requires the target-update counters (`gradient_updates` on the
...
skipped`, both terms readable from the agent. **The counter is what makes 0059
...
running total and the site label each time. ADR 0065 §Decision 3's mechanics are
adopted **wholesale**, not re-derived: same field shape, same
...
**Reaffirmed verbatim from ADR 0056 §Decision 3, because this is the clause most
at risk from the change above:** the **skip re-fires on every non-finite
...
**not** `#[cfg(test)]`. ADR 0056 §Decision 1's `#[cfg(test)] warning_fired()` is
**deleted, not reimplemented under a new name**. This mirrors 0065 §Decision 1's
`dropped()`, and for the reason 0065 gave: it is operationally useful outside
tests, being the term each agent's public accessor reads.
...
this counter. **ADR 0059 §Decision 4's unconditional advance is not changed, not
weakened, and not to be "fixed" in light of this counter.** 0059 already rejected
gating the cadence on applied updates, by name, in its own §Alternatives ("it
makes the cadence a function of run health"). This ADR supplies the *second term*
so the first can stay unconditional and still be interpretable — the counter
exists precisely so nobody is tempted to reopen 0059.
...
those have one loss site, so their per-site accessor from §Decision 2 **already
is** the aggregate, and adding a second name for the same `u64` would be pure
...
codebase: ADR 0065 §Context recorded that two of six `remember` sites were
...
`AgentStats`.** Therefore **ADR 0071 §Reopen trigger 1 does not fire, and is not
weakened** — `AgentStats` still carries exactly eleven accessors after this
change, and the next addition to it still owes the bundled-struct argument ADR
0071 demands.

The reason is §Context 4's, and it is a *units* reason first: `AgentStats` is
episode-scoped (`record(entry: T)` on episode end), while a loss skip is
per-update and per-loss-site. #346's proposed destination is a **scope error**,
not merely an ADR collision — which matters, because "blocked by a reopen
trigger" invites someone to argue the trigger away, and there is nothing to argue
away here. Even with unlimited accessor budget on `AgentStats`, the counter
would not belong there.

### 4. One canonical metric row, `skipped_updates` — repealing 0056 §3's "not a NaN metric"
...
This is the direct repeal of 0056 §Decision 3's "the `warn!`, **not a NaN
metric**, is the surfacing mechanism". Note what the repealed clause actually
conflated: 0056 was rejecting *poisoning an existing mean with a NaN* — which
this ADR agrees with, and which stays rejected (0056 §3's exclusion of skipped
values from epoch-mean accumulators is untouched). It then over-generalised from
...
(d) **§Decision 2's accessors are already the programmatic per-site API.** 17
rows would be a second, weaker copy of a surface that already exists, kept in
sync with it by hand. Two representations of one fact, one of which the compiler
checks and one of which it does not.

**On #184 as precedent.** ADR 0056 §3 leaned on #184 ("which added no stats
field") to justify log-only surfacing. That analogy is the weaker one here, and
saying so is part of the repeal: #184 guarded a **single closed-form scalar
step** (SAC's α optimizer) where occurrence and rate nearly coincide — one site,
one hand-rolled update. This guard spans **17 sites across 8 agents**, and the
quantity whose *magnitude* is the operational fact is the aggregate over them.
This is the same reasoning ADR 0065 §Decision 3 and §Literature used to reject a
latch for the reward guard: where the failure can recur at a rate, the rate is
the observable. #184 remains good precedent for #184's own guard, which is why it
is untouched.
...
- **ADR 0059 §Decision 4 becomes legible.** `applied = attempts − skipped` is
  recoverable without parsing logs, which removes the standing temptation to
  reopen 0059 and gate the cadence on applied updates.
- **The two guards now share one schedule and one shape**, adjacent in one file.
  A reader learns the pattern once.
- **The aggregate has exactly one definition per agent** (§Decision 3), so a
  future sixth guard site cannot be silently omitted from one train loop and
  present in another.
...
- **`FiniteRewardGuard`'s rustdoc rationale is now historically wrong** and is
  corrected in the same PR (§Context 2). Without that, the crate would ship two
...
- **Honest limit, carried forward verbatim in force from ADR 0056
  §Consequences: this remains a loss-level *proxy*.** It counts **skips, not NaN
...
- **A field/accessor on `AgentStats`** — the issue title's literal proposal.
  ...
  record without an aggregation the caller cannot undo (§Context 4). ADR 0071
  §Reopen trigger 1 independently fences the destination. Two reasons, and the
  scope one would stand alone.
- **Seventeen registry rows, one per site.** Rejected on §Decision 4's four
  grounds: agent-prefixed names breaching the table's name-by-role convention;
  13 of 17 panels structurally absent on every run; per-site attribution already
  served better by the `warn!`'s `site` field *and message*; and §Decision 2's
  accessors already being the programmatic per-site API, so the rows would be a
  hand-synchronised second copy.
...
- **Per-interval emission.** §Decision 5: the value would be a function of
  `log_every`, a logging parameter, making two runs of the same config
  incomparable; and first-differencing recovers the interval view from a
...
- **Fusing the warning schedule into the return value** (skip only when warning).
  ...
  §Decision 1 reaffirms 0056 §3 verbatim for this reason.
- **A `debug_assert!` or config gate on the guard.** Rejected by name for the
  fifth time — ADR 0056 §Decision 4 settled it, 0065 §Decision 5 adopted the
...
- **A bundled per-update health struct** (`update_health() -> UpdateHealth { … }`)
  ...
  rejected on principle, and §Reopen trigger 2 says when to re-argue it.
...
1. **Anything in this crate begins to *branch* on a skip count.** `skipped`
   ...
   **because it is a reported statistic, not a predicate** — the same standing
   ADR 0071 §Decision 7 gives `non_finite_episodes` and §Reopen trigger 3 fences
   there. A `u64` incremented once per skipped update cannot realistically reach
...
3. **The per-site / aggregate split is collapsed** in either direction — an
   ...
   calling the agent's aggregate. §Decision 3 is the record of why the split
   exists and what the single aggregate protects; collapsing it silently
   reintroduces the "one train loop forgot the new site" failure.
...
- Issue **#346** — "[rl] add a skip counter to `AgentStats` for the finite-loss
  ...
  the guard and on the agents, not on `AgentStats` (§Context 4, §Decision 3).
- Issue **#318** / ADR [0056](0056-non-finite-loss-skip-and-warn-guard.md) —
  **partially superseded**: §Decision 1 (type shape) and §Decision 3 (the
  surfacing clause and the one-shot latch). §Decision 2, §4, §5, §3's *skip*
  semantics, and the whole of its §Consequences remain in force; the skip
  semantics are reaffirmed verbatim in §Decision 1 and the honest limit is
  restated in §Consequences.
- ADR [0065](0065-non-finite-reward-is-dropped-at-replay-ingestion.md) — the
  ...
  (§Decision 1), and the `dropped()` non-test-gated accessor precedent
  (§Decision 2). Its §Decision 3 is unchanged; only its rustdoc characterisation
  of the *loss* guard is now historical (§Context 2), corrected in code.
- ADR [0059](0059-target-update-cadence-counts-gradient-updates.md) §Decision 4 —
  the unconditional attempt-counter advance this counter complements and does
  **not** change; `applied = attempts − skipped` (§Decision 2, §Context 5).
...
- ADR [0071](0071-best-score-latches-plus-infinity-the-finite-best-is-additive-and-counted.md)
  §Reopen trigger 1 — the twelfth-accessor fence on `AgentStats`, which this ADR
  explicitly does **not** fire and does **not** weaken (§Decision 3). Its
  §Decision 7 saturation reasoning is the model for §Reopen trigger 1 here.
...
- ADR [0015](0015-shared-typed-metric-registry-crate.md) — the typed registry
  §Decision 4's single row joins, and the `names_are_unique` constraint ground
  (a) rests on.
- Issue **#184** (`sac_alpha.rs`) — the precedent ADR 0056 §3 leaned on, and the
  weaker analogy here: one closed-form scalar step versus 17 sites across 8
  agents (§Decision 4). Its own guard and its 21 `warning_fired` references are
  untouched.
- Issue **#328** — finite-loss → NaN-gradient and grad-norm handling; still out
  of scope, and the reason the counter is a proxy (§Consequences).
- Issue **#352** / ADR 0065 — the reward-ingestion sibling whose rustdoc recorded
  this defect against its own neighbour (§Context 1).
...
- `crates/rlevo-reinforcement-learning/src/algorithms/shared.rs`:
  ...
  - `:593-597` — the `applied = attempts − skipped` composition with ADR 0059's
    counters.
  - `:629-635` — `check`'s `CRITICAL:` rustdoc reaffirming 0056 §3's skip
    semantics; `:636-662` — its body, with the unconditional `self.skipped += 1`
    at `:640` and the scheduled `warn!` at `:641-660`.
  ...
  - **Pre-change** `:650-654` — the loss guard's blind spot documented in-tree
    (§Context 1); **pre-change** `:664-669` — the "latches its warning once, and
    that is right" rationale this ADR overturns (§Context 2).
...
- The 17 `FiniteLossGuard::new` sites, tabulated in §Context 3:
  `ppo_agent.rs`, `ppg_agent.rs`, `sac_agent.rs`, `td3_agent.rs`,
...
- `crates/rlevo-reinforcement-learning/src/metrics.rs:126-143` —
  ...
  destination a units mismatch (§Context 4).
