...
exactly the density failure this ADR's own Context section (the population
argument), above, already rejected for the workspace-vs-crate question, one
level down. So the guard reads only the Site column of rows between
`### Documented Panic Contracts` and the first non-`|` line after it.
...
Settled in this ADR's own Context section. Every crate under `crates/*/src`
is scanned; the table is checked against all of it, not against the crate
that happens to define the ADR being written. `NON_PRODUCTION_CRATES`
(`rlevo-benchmarks`, `rlevo-benchmarks-report-client`, `rlevo-examples`,
`rlevo-test-support`) is not an exemption from scanning — a spec that
resolves *only* inside one of those crates is a **hard failure**, because
`docs/rules.md`'s Workspace Structure section fixes the dependency
direction (production crates never depend on the benchmark, example, or
test-support layer) and a panic contract that lives only there is a promise
about code no library user can reach. Zero rows hit this today.

### 2. Scope: the table's Site column, not `rules.md`'s surrounding Documented Panic Contracts prose

Settled in this ADR's own Context section, including the named cost:
`docs/rules.md:116`'s `C51Config` (should be `C51TrainingConfig`) is a
real, present citation error, and this guard will never see it, because it
lives in prose outside the table. Its successor — the next prose citation
that drifts — will not be caught by this guard either. That is a stated
limit, not an oversight.

### 3. Direction: dead rows only, never missing rows

Settled in this ADR's own Context section. #1108's direction remains
review-enforced. **The table is not thereby proven complete**, and this
ADR says so explicitly rather than leaving it to be inferred from five
green tests: a row can be accurate and the table can still be missing a
panic contract that belongs in it. The guard's name —
...
wrapped `for T` header (Step C, see this ADR's own Decision 7). For the
three trait-declaration rows the table names by trait rather than by
...
Following ADR 0062's own Decision 4 / ADR 0068's formula:
...
- **Blind to `#[cfg(test)]` regions**, deliberately — the alternative is a
  ...
  glob check in this ADR's own Decision 5 would go vacuous.
  `cfg_test_regions` fails loud on an unterminated or misindented region
  rather than silently swallowing the rest of a file.
...
`rlevo-test-support`. ADR 0062's own Decision 4 and ADR 0068's own
Decision 2 both rest on each source-text guard being independently
deletable when its premise stops holding; a shared helper would make
deleting any one of the three guards a cross-crate change, which is
exactly the friction that keeps a guard alive past its usefulness rather
than the friction that protects it.
...
1. A second real `UNRESOLVABLE_SITES` row, or the exemption list crossing
   10% of the table's rows (this ADR's own Context section) — the table
   has stopped being mostly nameable identifiers.
2. A row that needs to name something inside `rlevo-benchmarks`,
   ...
   a production item instead — a real instance of the `rules.md`'s
   Workspace Structure section dependency-direction problem this guard is
   built to catch as a hard failure, not a config error in the guard.
3. A glob row's match count dropping to one — today's zero-margin floor
   (this ADR's own Decision 5) means this is the most likely single-line
   change to break the guard, and the fix is always to name the surviving
   item, not to lower the floor.
4. Someone builds the #1108 direction (every panic site has a row) as a
   mechanical check. If that ever lands, this ADR's own Decision 3
   disclaimer — "not thereby proven complete" — should be revised to
   describe what the new check adds, not deleted.
...
- **#1085 cannot recur silently.** A row naming a deleted item now fails
  ...
  the table carefully.
- **The remediation ADR 0050's own Decision 8 wrote down and nobody
  executed is now enforced, not merely documented a second time.** The
  `with_alpha` / `with_capacity` rows this guard's own workspace scan found
  already cleaned up are the concrete instance; the mechanism generalizes to
  every future row.
...
- **Cheap to delete.** No production code depends on this guard's
  existence; if its assumptions stop holding (this ADR's own Assumptions
  and reopen-trigger section), removing it costs nothing beyond restoring
  the status quo ante.
...
- **Only one of the table's two failure directions is mechanized.** A row
  ...
  review. This ADR does not close that gap; this ADR's own Decision 3 says
  so in the guard's own module doc and here.
- **The table's surrounding `rules.md` Documented Panic Contracts prose is
  unchecked**, with a named, present instance of the cost:
  `docs/rules.md:116`'s `C51Config` citation (the type is
  `C51TrainingConfig`) is not caught by this guard and will not be,
...
- **The glob floor ships at zero margin.** Deleting either
  ...
  that is intended (this ADR's own Decision 5), but it means the very next
  refactor of either function is where this cost is first paid.
- **Text-based, not semantic**, with every limit that implies (this ADR's
  own Decision 8): defeatable by a comment, by cross-file reformatting, by
  a same-file name collision. It catches the accident, not the adversary —
  the correct threat model for this failure class, and the same one 0062
  and 0068 accept.
- **A third copy of `cfg_test_regions` now exists**, by deliberate choice
  (this ADR's own Decision 9), rather than a shared helper. Any future
  improvement to the region scanner (a new terminator shape, say) must be
  applied to three files by hand, or the three guards drift apart in what
  they can parse.
...
- **Crate-scoped guards, one per crate, mirroring ADR 0068.** Rejected on
  the population argument in this ADR's own Context section (its first
  subsection): the table's rows are already scattered across four
  production crates by construction, so a per-crate guard would need to
  either duplicate the table-parsing machinery per crate or split the
  table by crate — a maintenance burden the single workspace-shaped
  population does not justify. 0068's precedent transfers in reasoning,
  not in answer, because the two populations are shaped oppositely (this
  ADR's own Context section).

- **Mechanizing the #1108 direction alongside this one.** Rejected for now
  on the density argument in this ADR's own Context section: distinguishing
  "a panic site that is a documented contract" from "a panic site that is
  an internal invariant" across
  `panic!`/`assert!`/`unwrap`/`expect`/indexing/overflow is a judgement
  this guard's text-scanning approach cannot make reliably, and a guard
  that makes it badly earns itself a deletion rather than trust. Left as a
  review-enforced convention, and named on the reopen list (this ADR's own
  Assumptions and reopen-trigger section, item 4) rather than closed off.

- **Extending scope to the table's surrounding `rules.md` prose (Error
  Handling section; the original citation also named a second, numbered
  rules.md section that does not resolve to anything panic-contract-related,
  noted here rather than silently dropped).** Rejected on the same density
  argument: illustrative backticked mentions
  ...
  enough to separate them would itself become the noise ADR 0068's own
  Alternatives-considered section warned a 94%-allowlist guard becomes.
  The known cost (`C51Config`/`C51TrainingConfig` at `rules.md:116`) is
  accepted and recorded rather than chased with a wider scan.
...
- **A single-line `impl … T` scan for type-tier Step C, dropping the
  ...
  type's declaring file. See this ADR's own Decision 7.
...
- **A tier tripwire requiring a live row through Step C.** This was the
  ...
  row reaches Step C today (this ADR's own Decision 7). Rejected in favor
  of pinning the wrapped-header mechanism directly with a dedicated test,
  because a tier tripwire on Step C would assert a fact about which file a
  method currently lives in, and any legitimate refactor moving a method
  into its type's declaring file would make that tripwire fail for a
  reason having nothing to do with the guard's actual purpose.

- **Factoring `cfg_test_regions` into `rlevo-test-support`.** Rejected on
  ADR 0062's own Decision 4 / ADR 0068's own Decision 2's shared reasoning:
  each source-text guard must stay independently deletable, and a shared
  helper turns deleting one guard into a cross-crate change. See this
  ADR's own Decision 9.
...
- **A glob floor of one match instead of two.** Rejected: a glob matching
  ...
  the pair the two globs also cover individually. See this ADR's own
  Decision 5.
...
- Issue **#1085** — a module deleted, two `Builder with_*` rows outliving
  it by months. The remediation ADR 0050's own Decision 8 wrote down
  (`with_alpha` retires, `with_capacity` renames) and did not enforce.
...
- ADR [0050](0050-replay-strategy-seam.md)'s own Decision 8 — the prior
  remediation for the `with_alpha`/`with_capacity` rows, written down and
  unexecuted until this guard existed to enforce it.
- ADR [0062](0062-grid-layout-fidelity-and-no-dead-rng.md)'s own Decision
  4 — the precedent that a source-text guard's **scope** is itself an
...
- ADR [0068](0068-bounds-strictness-enforcement-is-crate-asymmetric.md) —
  ...
  (this ADR's own Decision 9).
- `docs/rules.md`'s Error Handling section, "Documented Panic Contracts"
  subsection — the table this guard checks, and (uncaught, this ADR's own
  Decision 2) the surrounding prose, including the
...
- Code —
  ...
  — `History`'s wrapped impl headers, the live example behind this ADR's
  own Decision 7.
- Code —
...
- Code — `crates/rlevo-reinforcement-learning/tests/bounds_strictness_guard.rs:811-865`,
  ...
  `cfg_test_regions` copies this guard's is a third of (this ADR's own
  Decision 9).
