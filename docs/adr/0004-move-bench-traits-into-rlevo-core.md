...
**Update (2026-09-19):** the preceding annotation's "points #1–#7 all remain active" no longer holds in full. ADR [0077](0077-remove-benchenv.md) **supersedes the remainder of this ADR as it applies to the `crates/rlevo-benchmarks/src/env.rs` → `crates/rlevo-core/src/evaluation.rs` move**: that move (decision point #1's first line), its typed `BenchError` (point #4), the `rlevo_benchmarks::env` shim (part of point #5), and the adapter rewrite in point #7 are all retired, because `BenchEnv`, `BenchStep`, and `BenchError` themselves were deleted — `BenchAdapter` with them. **Everything else in points #1–#7 remains active**: the `fitness.rs` and `util/seed.rs` moves, point #2's `util` promotion, point #3's `Metric` / `MetricsProvider` relocation, the `agent` and `seed` shim modules, and point #6's dependency-edge rework. `rlevo-core::evaluation` still exists and now holds `GenerationProbe`; its module docs record what was removed and why.
