...
**Supersedes nothing. Extends ADR 0070 §Correction (b) and §Reopen trigger 3,
and ADR [0065](0065-non-finite-reward-is-dropped-at-replay-ingestion.md)
§Decision 4** — the same "extends, does not supersede" relation 0070 took to
0065 and ADR [0069](0069-sanitized-fitness-is-reduced-in-f64.md) took to ADR
[0034](0034-fitness-hygiene-chokepoint-convention.md). **0065 and 0070 both stay
`active`,** and the following clauses of theirs remain in force verbatim:

- 0065 §Decision 4's **"Episode return is deliberately left poisoned"**
  (`0065:162-177`). Untouched. The eight `episode_reward +=` sites still
  accumulate unconditionally, and this ADR depends on that being true (§Context
  4).
- 0070 §Decision 1 — **`avg_score` is unchanged**, and its contract lives on its
  own rustdoc.
- 0070 §Decision 2 — **the pair is the decision**: a hardened statistic without
  its exclusion count is silent sanitization.
- 0070 §Decision 3 — **the predicate is `is_finite()`**, not `!is_nan()`.
- 0070 §Decision 4 — **`None`, not a fabricated value**, for the empty and
...
two private fields (`:57-60`).** 0070 §Decision 5 committed to touching neither.
That commitment was made against a *windowed* counter and rested on one specific
objection — eviction coupling — which does not exist for a lifetime statistic.
§Decision 6 discharges it in full.

This ADR also carries a **correction to ADR 0070**; see §Correction to ADR 0070.

## Context

### 1. What #1078 got right, and the one thing it left open

Recording this plainly, because it is the opposite of §Context 1 in ADR 0070:
**every load-bearing claim in #1078 verified.**
...
- The severity ranking against #409 is correct: this is **strictly worse** than
  the `avg_score` case, which recovers within one `window_size` (§Context 3).
...
and the wrong state for a codebase, and §Decision 1 answers it: the latch stays
because it is true, and a sibling ships because "true" is not the same as
"usable".

### 2. This is ADR 0070 §Correction (b) coming due
...
0070 §Reopen trigger 3 then pre-authorised both this ADR's **existence** and its
**scope**:

> **The `best_score` `+∞` latch (§Correction (b)) is fixed in a way that
> introduces its own finite/raw pair** … At that point `AgentStats` carries two
> independent finite/raw pairings, and they should be unified in naming,
...
stated deliverable, and §Decision 4 discharges it explicitly. What this ADR adds
that 0070 could not is the *policy answer* — plus one piece of context 0070 did
not have (§Context 4).
...
scan. A latched `+∞` is therefore both permanent and underivable, and 0070
§Decision 5's derive-on-call shape — the obvious thing to reach for by analogy —
is not merely inefficient here, it is **wrong**, in the sense of computing a
different quantity. It is pinned as wrong by
`finite_best_score_survives_eviction` (`metrics.rs:1160`), whose doc comment
names that mutant as "the most important mutant of the whole change".

### 4. `+∞` is far more reachable than ADR 0070 §Context 4 said
...
0070 §Context 4 rested on a single path: `f32` saturation of `episode_reward`
over a long episode of entirely finite rewards. That path is real, and it
remains the one no guard can ever see. But there is a **second, far more
reachable path**, and it runs straight through ADR 0065's design:
...
reward_f32` unconditionally **and by design**, because 0065 §Decision 4 requires
it — a dropped transition must still surface in the reported curve:
...
|---|---|---|---|
| non-finite env reward (0065 §D4) | `>= 1` | yes (decade schedule) | yes |
| `f32` accumulator saturation (0070 §Context 4) | `0` | no | yes |
...
**One 0070 §Context 2 claim is now spent, and must not be re-asserted.** 0070
...
all eight sites now carry it, as the table above shows. §Correction (b) records
this so the table above is not misread as contradicting 0070.
...
selection criterion is a silently wrong model. §Reopen trigger 2 puts a fence
around that.

### 6. No existing gate would have caught this, and none was blinded

ADR 0070 §Context 5 turned on a mechanical net: `assert_improves_over_random`
...
0070 §Rejected alternatives ruled out for the mean. #1078's first option is
adopted in full, as far as it goes.

This is the same move as 0070 §Decision 1, with one point sharper. 0070 found
...
0070 §Correction (b) had to narrow. A reader following the documentation was
...
"this is not filtered, and must not be", both reachability paths from §Context
4, the `−∞` asymmetry, and a pointer to the hardened pair.
...
0070 §Decision 2's reasoning is **inherited, not re-argued**: a hardened
...
extra (§Literature, third bullet).

### 3. The predicate is `is_finite()`

Adopted from 0070 §Decision 3 rather than re-argued: `±∞` is as destructive as
...
no finite score has ever been recorded. 0070 §Decision 4's rejections are
inherited unchanged:
...
**The unification clause, discharging 0070 §Reopen trigger 3 by name.** That
trigger required the two finite/raw pairings to "be unified in naming,
predicate, and empty-case behaviour rather than each arguing its own case".
As of this ADR:
...
a gap in the unification — see §Decision 5.
...
feature.** It is the same split ADR 0065 §Decision 1 drew for the six agents'
`dropped_transitions()`, which is likewise a monotone lifetime counter
...
### 6. `record` gains state, and ADR 0070 §Decision 5's objection does not transfer

This is the clause a future reader will most want, because on its face this ADR
does the thing 0070 declined to do.

0070 §Decision 5 reads: "`AgentStats` gains **no field**, and `record` … is not
...
which set it (§Context 3). 0070 anticipated exactly this in its own §Reopen
trigger 2: "a lifetime statistic cannot be derived from state that has been
evicted, so §Decision 5 falls and `record` must maintain state after all."

So this is not a reversal of §Decision 5. It is §Decision 5 applied to the case
it explicitly scoped itself out of.
...
1. **The invariant is against a `usize`.** `non_finite_episodes() <=
   total_episodes()` is the property §Decision 7 establishes by construction,
   and `total_episodes` is a `usize` (`metrics.rs:52`). A `u64` counter would
...
3. **`dropped_transitions()`'s `u64` is not a workspace convention to follow.**
   ...
   invariant. Copying its width here would be cargo-culting a type the way 0070
   §Decision 4 declined to cargo-cult 0069's `−∞` constant.
...
after 0070 §Decision 6 widened the mean's accumulator.
...
- **Filter inside `best_score` itself** — the one-line "obvious" fix, and the
  ...
  3. **It silently changes the meaning of an existing public accessor.** Every
     ...
     mechanism ADR 0070 §Rejected alternatives ground (2) rules out generally:
...
- **A count derived from the window**, i.e. 0070 §Decision 5's shape reused for
  `non_finite_episodes`. Rejected on two independent grounds, either fatal:
...
- **`!is_nan()` as the predicate.** §Decision 3: `f32::max` already handles the
  `NaN` half, so this predicate produces an accessor observationally identical
  to the one it is meant to harden at every input but `+∞`.
...
- **A bundled metrics struct**, e.g. `finite_score_summary() ->
  ...
  **And a verdict on 0070 §Reopen trigger 1, since a reader may think this
  ...
  eleven, which is a real cost (§Consequences), and this ADR answers it with a
  tighter successor trigger rather than a struct.

- **Clamping `+∞` to `f32::MAX`**, the shape ADR 0034 §Decision 4 uses for
  fitness. Rejected specifically *because* this is a maximum. Clamping
...
- **A `debug_assert!` or config-gated check.** Rejected by name for the fourth
  time — ADR [0056](0056-non-finite-loss-skip-and-warn-guard.md) §Decision 4
  settled it ("the host read already exists, so a gate would strip the guard
  from exactly the long release runs that diverge"), 0065 §Decision 5 adopted
  that ruling unchanged, 0070 cited it a third time. The argument is *stronger*
  here than in either: §Context 4's saturation path requires a long episode, so
  long release runs are not merely where the failure is likeliest, they are the
  only place one of the two paths exists at all. A release no-op protects
  nothing.
...
- **A caller who needs a usable lifetime maximum has one**, with the exclusion
  ...
  paths from §Context 4 — including the one no ingestion guard can see.
...
- **0070 §Reopen trigger 3 is discharged rather than deferred** (§Decision 4):
  the two finite/raw pairs agree on prefix, predicate, empty case,
  all-non-finite case, and return type.
...
- **`record` now carries state that only `record` can maintain**, which 0070
  §Decision 5 explicitly declined to introduce. §Decision 6 explains why the
  specific objection does not transfer, append-only-ness bounds the risk, and
...
- **Nothing mechanically forces a caller to read `non_finite_episodes()`
  alongside `finite_best_score()`.** §Decision 2 argues the pair is indivisible;
  `#[must_use]` does not make it so, because it says nothing to a caller who
...
- **`best_score()` still latches at `+∞`, forever.** This ADR **does not fix the
  ...
  maximum is rejected in §Rejected alternatives on three grounds — but anyone
  arriving here from the issue expecting the number to change should be told
...
- ADR 0065's `FiniteRewardGuard` (`shared.rs:681,715-716`), its decade `warn!`
  ...
  enumerated in §Context 4 are all byte-for-byte untouched.
...
1. **A twelfth accessor of any kind on `AgentStats`** — deliberately tighter
   than 0070 §Reopen trigger 1's "a third windowed mean". At eleven, with two
...
2. **A checkpoint-on-best-score feature lands.** It must read
   `finite_best_score()`, not `best_score()` — §Context 5 — and the question of
   whether a run that produced a `+∞` episode should be *checkpointable at all*
   is a policy question this ADR does not answer and should not be assumed to.
   It deserves its own record.
3. **Anything in this crate begins to branch on `non_finite_episodes`.** The
   saturation rationale in §Decision 7 rests explicitly on `record`'s own
   statement that "nothing in this crate branches on the counters, they are
...
4. **`episode_reward` becomes `f64`, or a per-episode finiteness guard is added
   to the training loops.** The first removes §Context 4's saturation path (an
   `f64` accumulator will not saturate over any realistic episode); neither
   removes the env-reward path, which is required to stay open by 0065
   §Decision 4. A change that closes one path and is described as closing "the"
   path is the misreading this trigger exists to catch.
5. **Anyone proposes collapsing `non_finite_episodes` and
   `non_finite_recent_len` into one accessor.** This ADR (§Decision 5) is the
   record of why there are two; the collapse forces a choice between "did this
   run ever go bad?" and "is it bad now?", and pairs a lifetime maximum with a
   windowed count.

## Correction to ADR 0070

ADR 0070 is **not edited** (`docs/rules.md:649`; `docs/adr/README.md:3-6`). The
corrections are carried here, following the precedent ADR 0066
§"Correcting the record" (`0066:117-142`, the correction itself at `0066:133-137`)
and ADR 0067 §"Correction to ADR 0065" (`0067:201-215`) set for exactly this
situation — and which 0070 itself reused for 0065. As at `0066:125` and in
0070's own correction section, **all line numbers resolve against the commit
that wrote them.**

### (a) Stale citations — and it is the exact failure 0070 warned about one section earlier

0070 §Correction (b) pins the two `best_score` NaN tests at `metrics.rs:402,447`.
Those are **pre-diff coordinates**: at `acd7b97^` the `fn` lines were 402 and
447, at `acd7b97` they were 587 and 633, and under the #1078 diff now in the
working tree they are:
...
0070 §Correction (a) had *just* written, one section earlier:
...
0070's §References carry the same staleness throughout, all from the #1078 diff
shifting every coordinate below `:56`:
...
| `metrics.rs:242-253` (`avg_score` body, from 0070 §Correction (a)) | **`:429-440`** | `avg_score`'s body |
...
| `metrics.rs:93-99` (the `window_size` ceiling, 0070 §Decision 5) | **`:97-103`** | the `MAX_BUFFER_CAPACITY` assertion |
...
Non-`metrics.rs` citations in 0070 §References were re-resolved as well and are
**still accurate**: `strategy.rs:357-370` (`mean_fitness`/`broken_count`),
...
### (b) One 0070 §Context 2 claim is spent, not wrong

0070 §Context 2 is headed "One of eight training loops explains itself" and
records that only DQN carried the `DELIBERATE:` comment while "the other seven
sites in the table above are a bare `episode_reward += reward_f32;` with no
comment, no ADR citation, and nothing to stop a reader concluding it is an
oversight". Its §Consequences then lists closing that gap as future work: "the
seven undocumented `episode_reward +=` sites stop being an invitation to re-file
#409".

**Commit `acd7b97` closed it.** All eight sites now carry the `DELIBERATE:`
block, at the line numbers tabulated in §Context 4. The claim was true when
written and is now spent. It is recorded here so that this ADR's §Context 4 —
which enumerates the same eight sites *with* their comments — is not read as
contradicting 0070, and so that the asymmetry is not re-asserted by a future
reader who finds 0070 before finding this.
...
`f32::max`, which ignores a NaN operand …"), and **0070 §Correction (b) already
narrowed it** to "cannot corrupt the best-ever record *with a `NaN`*", correcting
the `metrics.rs:71` coordinate at the same time. That narrowing is correct and
this ADR is the follow-up it anticipated. Nothing further in 0065 requires
amendment; §Decision 4's "Episode return is deliberately left poisoned"
(`0065:162-177`) is not merely preserved but **relied upon** by §Context 4.

## Literature

Deliberately short, and built on ADR 0070 §Literature and ADR 0065 §Literature
...
- **IEEE 754 `maxNum`, and the half-guarantee it gives.** The standard
  ...
  **why the `NaN` half of this defect does not exist** — the fold discards a
  ...
  0070 §Literature had already named the complementarity — "clipping neutralises
  `±∞` but not `NaN`; `f32::max` neutralises `NaN` but not `+∞`" — and filed the
  second half as a live gap. **This ADR is that sentence coming due.** The
  general ruling is ADR 0066's, unchanged: where correctness depends on a
  non-finite value's fate, pin it with an explicit predicate rather than relying
  on an operation's implicit behaviour. §Decision 3 is that ruling applied.

- **Order statistic vs. moment — new to this ADR, and the formal content of
  §Decision 6.** A mean is a **moment**: a reduction over all terms, in which one
  ...
  **order statistic**: it selects a single term, so an extreme value is diluted
  ...
  (§Decision 6), a monotone rather than self-healing count (§Decision 5), and a
  hardened accessor that is a genuinely different computation rather than a
  filtered view of the same one.

- **Missing-not-at-random, reused rather than re-derived** (0070 §Literature;
  0065 §Decision 3). Excluding non-finite episodes is statistically inert if
  their cause is policy-independent numerical noise, and systematic if the
  excluded episodes correlate with the states the policy actually visits — the
  realistic case, where the agent's own improving behaviour drives it into the
  regime that saturates the return. No property of the estimator distinguishes
  the two regimes; only telemetry from the run can. That is the load-bearing
  reason `non_finite_episodes()` is a **condition** of `finite_best_score()`
  existing (§Decision 2) rather than an ergonomic extra, and it binds harder here
  than for the mean: a hardened maximum, unlike a hardened mean, carries no
  trajectory from which a reader could infer the frequency for themselves.
...
- Issue **#1078** — "[rl] `AgentStats::best_score` latches at `+∞` permanently,
  ...
  analysis is adopted in full (§Context 1) and the policy choice it declined to
  make is made in §Decision 1.
- Issue **#409** / ADR
  [0070](0070-avg-score-transits-non-finite-scores-the-hardened-mean-is-additive.md)
  — **extended and corrected**. §Decisions 1-4 are inherited verbatim; §Decision
  5's no-stored-state ruling is scoped rather than reversed (§Decision 6); its
  §Reopen trigger 3 is discharged (§Decision 4) and its §Reopen trigger 1 is
  given a verdict (§Rejected alternatives); its stale `metrics.rs` citations and
  one spent §Context 2 claim are corrected here without editing it.
- ADR [0065](0065-non-finite-reward-is-dropped-at-replay-ingestion.md) —
  §Decision 1's monotone `dropped_transitions()` counter, mirrored by §Decision
  5's lifetime/windowed split; §Decision 4's "Episode return is deliberately
  left poisoned" (`0065:162-177`), which §Context 4 **relies on** and which
  remains in force verbatim. Its `best_score` parenthetical (`0065:178-180`)
  needs no further correction — 0070 §Correction (b) already narrowed it.
- ADR [0034](0034-fitness-hygiene-chokepoint-convention.md) §Decision 4 — the
  mean-plus-`broken_count` shape 0070 §Decision 2 mirrored and this ADR inherits
  (`crates/rlevo-evolution/src/strategy.rs:359,369`), and the `f32::MAX` fitness
  clamp §Rejected alternatives declines to reuse for a maximum.
- ADR [0061](0061-optional-facing-and-tensorconvertible-no-fabrication.md) — the
  no-fabrication rule behind §Decision 1, §Decision 4's rejection of
  `Some(0.0)`, and §Rejected alternatives ground (1).
- ADR [0069](0069-sanitized-fitness-is-reduced-in-f64.md) — the `f64`
  accumulator rule §Decision 9 shows has **no purchase** on a selection, and the
  `−∞` empty-case sentinel §Decision 4 again declines to adopt
...
- ADR [0056](0056-non-finite-loss-skip-and-warn-guard.md) §Decision 4 — the
  no-`debug_assert`/config-gate ruling, adopted by 0065 §Decision 5, cited by
...
- ADR [0023](0023-objective-sense-and-maximize-convention.md) — the
  ...
  `rlevo-reinforcement-learning` does **not** have, which is why §Decision 4
  returns `None`.
...
- `docs/rules.md:649` and `docs/adr/README.md:3-6` — the immutability rule that
  makes §Correction to ADR 0070 the only available mechanism.
...
- `crates/rlevo-reinforcement-learning/src/metrics.rs`:
  - `:11-12` — `PerformanceRecord::score`, "the primary scalar metric used for
    checkpointing and best-model tracking" (§Context 5).
  ...
  - `:56,58,60` — the `best_score`, `finite_best_score`, and
    `non_finite_episodes` fields; the latter two are the state §Decision 6 adds.
  ...
  - `:117-125` — `record`'s rustdoc, now naming all three saturating lifetime
    ...
    sentence §Reopen trigger 3 fences.
  ...
  - `:171-172` — "the best score is never evicted by the sliding window"
    (§Context 1).
  - `:174-216` — `best_score`'s new latch section (§Decision 1); `:217-220` —
    its unchanged body.
  ...
  - Tests: `:774` and `:820` (the two NaN tests 0070 §Correction (b) miscites);
    `:886` (`best_score_survives_eviction_while_average_does_not`); `:988`
    (`eviction_heals_the_non_finite_count`); `:1061`, `:1085`, `:1111`, `:1131`,
    `:1160`, `:1181`, `:1208`, `:1231`, `:1252`, `:1271` (the ten tests added by
    this change).
- `crates/rlevo-reinforcement-learning/src/algorithms/shared.rs:681` —
  ...
  `dropped() -> u64`, the different quantity §Decision 8 declines to match.
...
- The eight `episode_reward +=` sites and their `DELIBERATE:` comments,
  tabulated in §Context 4: `dqn/train.rs:129,137`, `ppo/train.rs:194,204`,
  `ppg/train.rs:135,148`, `sac/train.rs:122,130`, `td3/train.rs:106,114`,
  `ddpg/train.rs:121,129`, `c51/train.rs:122,130`, `qrdqn/train.rs:115,123`.
- `crates/rlevo-test-support/src/assert.rs:22-35`
  ...
  `avg_score` and **never** `best_score` (§Context 6); fed from
  `crates/rlevo-test-support/src/macros.rs:62,78`.
- The two external reads of `best_score()`:
  `crates/rlevo-examples/examples/book/ch03_dqn_cartpole.rs:179` and
  `crates/rlevo/tests/integration_test.rs:418` (§Context 5).
