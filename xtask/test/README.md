...
| `heavy` | The `#[ignore]`d suite, release profile, **minus acceptance** | minutes | `weekly-tests.yml` `regression`, Sundays 02:00 |
| `acceptance` | Full-solve convergence runs | **hours** | `weekly-tests.yml` `acceptance`, 350-min timeout |
...
cargo xtask test acceptance qrdqn_integration
...
`heavy` is the merge-gate tier and should finish in minutes. Folding in a
500 000-step full-solve run makes it hours, which is the same unnamed-scope
trap this runner exists to close — the caller asks for "the heavy tests" and
gets something four orders of magnitude larger than intended.

The split also maps the tiers one-to-one onto the two jobs `weekly-tests.yml`
already had, so no test name appears in YAML any more: `heavy` emits its own
`--skip` for every acceptance test in the binary it is running.

**The acceptance list is curated, not enumerated** — deliberately, and against
the rule used everywhere else here. These tests are generated by
`rl_learning_test!`, so the name never appears as `fn <name>` in any source
file and no grep can find them. A new acceptance test must be added to
`acceptance_units` by hand; nothing will notice if it is not, and it will
quietly run inside `heavy` instead, taking that tier back to hours.
